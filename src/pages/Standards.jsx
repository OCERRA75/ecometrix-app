...
// M17.2 — Tabla comparativa de estándares de sostenibilidad
...
const STANDARDS = [
  {
    ...
    features: {
      'Alcances 1, 2 y 3': true,
      'Certificación externa': false,
      'Reporte público': false,
      'Obligatorio UE': false,
      'Incluye cadena de valor': true,
      'Targets de reducción': false,
      'Reporte financiero integrado': false,
      'Soporte PYME': true,
    },
  ...
  },
  {
    ...
    sectores: ['Empresas con operaciones en Europa'],
    descripcion: 'Corporate Sustainability Reporting Directive — directiva europea que obliga a reportar sostenibilidad usando los European Sustainability Reporting Standards (ESRS). Afecta proveedores de empresas europeas.',
    features: {
      'Alcances 1, 2 y 3': true,
      'Certificación externa': true,
      'Reporte público': true,
      'Obligatorio UE': true,
      'Incluye cadena de valor': true,
      'Targets de reducción': true,
      'Reporte financiero integrado': true,
      'Soporte PYME': false,
    },
  ...
  },
  {
    ...
    año: '1997 / v. 2021',
    tipo: 'Marco de reporte',
    color: 'amber',
    logo: '📊',
    aplicable: { micro: false, pequena: true, mediana: true, grande: true },
    sectores: ['Todos — énfasis en impacto social'],
  ...
  {
    id: 'sbti',
    nombre: 'Science Based Targets',
    org: 'SBTi',
    año: '2015',
    tipo: 'Marco de objetivos',
    color: 'green',
    logo: '🎯',
    aplicable: { micro: false, pequena: true, mediana: true, grande: true },
    sectores: ['Todos'],
...
const SECTOR_OPTIONS = ['Todos', 'Manufactura', 'Retail', 'Servicios', 'Construcción', 'Agro']
const SIZE_OPTIONS = [
    ...
    <div className={`card cursor-pointer transition-all ${expanded ? 'ring-2 ring-brand-200' : ''}`} onClick={onToggle}>
      ...
      {expanded && (
        <div className="mt-4 pt-4 border-t border-border space-y-4">
          ...
          <div className={`rounded-xl p-3 ${c.header} border ${c.badge.split(' ')[2]}`}>
...
export default function Standards() {
  ...
  const filtered = sizeFilter === 'all'
    ? STANDARDS
    : STANDARDS.filter(s => s.aplicable[sizeFilter])
                  ...
                  <tr key={feature} className={i % 2 === 0 ? 'bg-surface-secondary' : 'bg-white'}>
                    ...
                    {filtered.map(std => (
                      <td key={std.id} className="p-4">
                        <FeatureIcon value={std.features[feature]} />
                      </td>
                    ))}
                  </tr>
                ))}
